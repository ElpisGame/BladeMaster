...
contract Test {
    ...
    function verify3(uint256 _salt, address _nftAddress, uint256 _nftId, address _token, uint256 _amount, bytes calldata signature) public view returns(bool){
        bytes32 hash = keccak256(abi.encodePacked(_salt, _nftAddress, _nftId, _token, _amount));
        bytes32 prefixedHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", hash));
        return (msg.sender == prefixedHash.recover(signature));
    }
